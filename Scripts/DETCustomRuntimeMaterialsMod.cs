...
namespace DETCustomRuntimeMaterials
{
    [Serializable]
    ...
    [ImportedComponent]
    public class DETCustomRuntimeMaterialsMod : MonoBehaviour
    {
        ...
        {
            //Debug.Log("DETCustomRuntimeMaterialsMod: Init called.");
            mod = initParams.Mod;
            var go = new GameObject(mod.Title);
            go.AddComponent<DETCustomRuntimeMaterialsMod>();        
        }

        private void Awake()
        {
            meshRenderer = GetComponent<MeshRenderer>();
            //Debug.Log($"[DETCustomRuntimeMaterials] Awake called for {gameObject.name}");
            LoadClimateMaterialSettings();
        }

        private void Start()
        {
            //Debug.Log("[DETCustomRuntimeMaterials] Start called");
        ...
        private void LoadClimateMaterialSettings()
        {
            ...
            string filePath = Path.Combine(Application.streamingAssetsPath, "DETCustomRuntimeMaterials", cleanName + ".json");
            //Debug.Log($"[DETCustomRuntimeMaterials] Attempting to load JSON from {filePath}");

            if (File.Exists(filePath))
            {
                string json = File.ReadAllText(filePath);
                //Debug.Log($"[DETCustomRuntimeMaterials] JSON loaded successfully, contents: {json.Substring(0, Math.Min(json.Length, 500))}...");

                fsResult result = _serializer.TryDeserialize(fsJsonParser.Parse(json), ref climateMaterialSettings);
                if (!result.Succeeded)
                    Debug.LogError($"[DETCustomRuntimeMaterials] Deserialization failed: {result.FormattedMessages}");
                else
                    Debug.Log("[DETCustomRuntimeMaterials] Deserialization succeeded");
            }
            else
            {
                Debug.LogError("[DETCustomRuntimeMaterials] JSON file for material settings not found");
        ...
        private void UpdateMaterialBasedOnClimateAndSeason()
        {
            ...
            if (definitions == null || definitions.Length == 0)
            {
                //Debug.LogError("[DETCustomRuntimeMaterials] No definitions found for the current climate and season.");
                return;
            }
            ...
            {
                //Debug.LogError("[DETCustomRuntimeMaterials] No valid materials found for the current climate and season.");
            }
        }
