...
namespace DETCustomRegionMaterials
{
    [Serializable]
    ...
    [ImportedComponent]
    public class DETCustomRegionMaterials : MonoBehaviour
    {
        ...
        {
            //Debug.Log("DETCustomRegionMaterialsMod: Init called.");
        }

        private void Awake()
        {
            meshRenderer = GetComponent<MeshRenderer>();
            //Debug.Log($"[DETCustomRegionMaterials] Awake called for {gameObject.name}");
            LoadRegionMaterialSettings();
        }

        private void Start()
        {
            //Debug.Log("[DETCustomRegionMaterials] Start called");
       ...
       private void LoadRegionMaterialSettings()
        {
            ...
            //Debug.Log($"[DETCustomRegionMaterials] Attempting to load JSON for '{cleanName}'");

            if (ModManager.Instance.TryGetAsset(cleanName + ".json", clone: false, out TextAsset jsonAsset))
            {
                string json = jsonAsset.text;
                //Debug.Log($"[DETCustomRegionMaterials] JSON loaded successfully, contents: {json.Substring(0, Math.Min(json.Length, 500))}...");

                fsResult result = _serializer.TryDeserialize(fsJsonParser.Parse(json), ref regionMaterialSettings);
                if (!result.Succeeded)
                    Debug.LogError($"[DETCustomRegionMaterials] Deserialization failed: {result.FormattedMessages}");
                else
                    Debug.Log("[DETCustomRegionMaterials] Deserialization succeeded");
            }
            else
            {
                Debug.LogError("[DETCustomRegionMaterials] JSON file for material settings not found");
        ...
        private void UpdateMaterialBasedOnRegion()
        {
            ...
            {
                //Debug.LogError("[DETCustomRegionMaterials] No definitions found for the current region.");
                return;
            }
            ...
            {
                //Debug.LogError("[DETCustomRegionMaterials] No valid materials found for the current region.");
            }
        }
